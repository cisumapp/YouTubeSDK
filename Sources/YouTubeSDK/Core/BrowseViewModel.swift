...
public final class BrowseViewModel {
    ...
    public func loadContent(for section: BrowseSection? = nil, refresh: Bool = false, source: String = "unknown") {
        ...
        if refresh {
            videoGroups = []
            subscribedChannels = []
            recommendedShortsInternalVideos = []
            loadedAt = nil
            enrichTask?.cancel()
            enrichTask = nil
        }
    ...
    public func loadMoreIfNeeded(lastInternalVideo: InternalVideo) {
        ...
        guard let lastGroup = videoGroups.last,
              lastGroup.videos.contains(where: { $0.id == lastInternalVideo.id }),
        ...
        do {
            ...
            case .home:
                ...
                if Task.isCancelled {
                    ...
                    let filteredRows = newRows.map { row -> InternalVideoGroup in
                        var copy = row
    ...
    private func mergeIntoFirstGroup(_ group: InternalVideoGroup) {
        if videoGroups.isEmpty {
            ...
            let newInternalVideos = group.videos.filter { seenIds.insert($0.id).inserted }
            videoGroups[0].videos.append(contentsOf: newInternalVideos)
    ...
    private func deduplicated(_ videos: [InternalVideo]) -> [InternalVideo] {
        var seen = Set<String>()
