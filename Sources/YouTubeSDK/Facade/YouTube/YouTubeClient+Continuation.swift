...
//                    writeSearchDebugDump(data, clientName: "web_continuation_\(endpoint)")
    ...
    func parseContinuationResults(from data: Data) -> YouTubeContinuation<YouTubeItem> {
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            print("[YouTubeSDK] parseContinuationResults: Failed to parse JSON from data (len=\(data.count))")
            return YouTubeContinuation(items: [], continuationToken: nil)
        }
        print("[YouTubeSDK] parseContinuationResults: JSON keys = \(Array(json.keys))")
        if let contents = json["contents"] as? [String: Any] {
            print("[YouTubeSDK] parseContinuationResults: contents keys = \(Array(contents.keys))")
        }
