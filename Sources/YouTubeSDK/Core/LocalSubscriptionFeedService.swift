...
//   Fallback: InnerTube fetchChannelInternalVideos (no auth, richer metadata, more rate-limit risk)
//
...
public struct LocalSubscriptionFeedService: Sendable {
    ...
    public func fetchFeed(
        ...
        var allInternalVideos: [InternalVideo] = []
        var metadataUpdates: [(channelId: String, name: String?, thumb: URL?)] = []

        await withTaskGroup(of: (channelId: String, videos: [InternalVideo], name: String?, thumb: URL?).self) { group in
            for channel in channels {
                ...
                group.addTask {
                    ...
                    // Fetch from network
                    let (videos, updatedName, updatedThumb) = await Self.fetchChannelInternalVideos(
                        channel: channelCopy,
                        forceRSS: forceRSS,
                        api: api,
                        session: sessionCopy
            ...
            for await result in group {
                allInternalVideos.append(contentsOf: result.videos)
    ...
    private static func fetchChannelInternalVideos(
        channel: LocalChannel,
        forceRSS: Bool,
        api: any InnerTubeAPIProtocol,
        session: URLSession
    ) async -> ([InternalVideo], String?, URL?) {
    ...
    private static func fetchViaRSS(channel: LocalChannel, session: URLSession) async -> RSSParseResult? {
        ...
        let enriched = uploads.videos.map { v -> InternalVideo in
    ...
    private static func fetchViaInnerTube(channel: LocalChannel, api: any InnerTubeAPIProtocol) async -> [InternalVideo]? {
        do {
            let group = try await api.fetchChannelInternalVideos(channelId: channel.id, continuationToken: nil)
