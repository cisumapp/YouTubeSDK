...
//  - LRU eviction: keeps at most `maxInternalVideoEntries` videos worth of data
//  - Active prefetch tasks are tracked so duplicates are skipped and
//    tasks can be cancelled when the user navigates away
//
// Usage:
//  1. Call `prefetch(videoId:...)` from a InternalVideoCardView `.task` modifier
...
public actor InternalVideoPreloadCache {
    ...
    private static let maxInternalVideoEntries = 30
    ...
    private let disk = InternalVideoDiskCache()
    ...
    public func consume(videoId: String) -> CachedInternalVideoData {
        touch(videoId)
    ...
    private func staleOrFresh<T: Sendable>(
        ...
        dataType: CachedInternalVideoData.DataType,
        into staleFields: inout Set<CachedInternalVideoData.DataType>
    ) -> T? {
