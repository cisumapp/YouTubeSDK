...
public enum OAuthTokenStorage {
    ...
    public static func save(_ token: OAuthToken) {
        ...
        YouTubeSDKConfig.storage?.save(string, key: OAuthKeychainKey.token)
    }

    public static func load() -> OAuthToken? {
        guard let string = YouTubeSDKConfig.storage?.load(key: OAuthKeychainKey.token),
    ...
    public static func delete() {
        YouTubeSDKConfig.storage?.delete(key: OAuthKeychainKey.token)
        YouTubeSDKConfig.storage?.delete(key: OAuthKeychainKey.userCode)
        YouTubeSDKConfig.storage?.delete(key: OAuthKeychainKey.deviceCode)
        YouTubeSDKConfig.storage?.delete(key: OAuthKeychainKey.codeExpiresAt)
    }

    public static func saveDeviceCode(userCode: String, deviceCode: String, expiresAt: Date) {
        YouTubeSDKConfig.storage?.save(userCode, key: OAuthKeychainKey.userCode)
        YouTubeSDKConfig.storage?.save(deviceCode, key: OAuthKeychainKey.deviceCode)
        let expiryString = String(expiresAt.timeIntervalSince1970)
        YouTubeSDKConfig.storage?.save(expiryString, key: OAuthKeychainKey.codeExpiresAt)
    }

    public static func loadDeviceCode() -> (userCode: String, deviceCode: String, expiresAt: Date)? {
        guard let userCode = YouTubeSDKConfig.storage?.load(key: OAuthKeychainKey.userCode),
              let deviceCode = YouTubeSDKConfig.storage?.load(key: OAuthKeychainKey.deviceCode),
              let expiryString = YouTubeSDKConfig.storage?.load(key: OAuthKeychainKey.codeExpiresAt),
