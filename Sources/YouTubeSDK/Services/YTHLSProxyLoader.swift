...
public final class YTHLSProxyLoader: NSObject, AVAssetResourceLoaderDelegate, @unchecked Sendable {
    ...
    public let webViewCookies: [HTTPCookie]
    ...
    public let selectedLanguageContentID: String?
    ...
    public let poToken: String?
    private let lock = NSLock()
    private var activeTasks: [ObjectIdentifier: URLSessionDataTask] = [:]

    public init(
        ua: String,
        nSolver: (unsolved: String, solved: String)? = nil,
        webViewCookies: [HTTPCookie] = [],
        selectedLanguageContentID: String? = nil,
        poToken: String? = nil
    ) {
        self.ua = ua
        ...
        let task = URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            if isPlaylist {
                if let text = String(data: data, encoding: .utf8) {
                    let rewritten = rewritePlaylist(text, baseURL: realURL)
    ...
    private func rewritePlaylist(_ m3u8: String, baseURL _: URL) -> String {
        ...
        if isMasterManifest {
            ...
            if audioGroupCount > 0 || variantCount > 0 {
                ...
                let extMediaCount = lines.count(where: { $0.hasPrefix("#EXT-X-MEDIA:") })
...
public extension YTHLSProxyLoader {
    ...
    static func makePlayerItem(
        ...
        let proxy = YTHLSProxyLoader(ua: ua, nSolver: nSolver, webViewCookies: webViewCookies, poToken: poToken)
